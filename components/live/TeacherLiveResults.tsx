...
export default function TeacherLiveResults({
  ...
  const galleryAnswers = useMemo(
    () => photoAnswers.filter((answer) => Boolean(answer.image_url)),
    [photoAnswers]
  );
  const shouldShowPhotoGallery = isPhotoMission && galleryAnswers.length > 0;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center">
        ...
        {shouldShowPhotoGallery ? (
          <div className="mt-10 w-full max-w-6xl rounded-4xl border border-white/20 bg-slate-950/30 p-4 shadow-xl backdrop-blur-md md:p-6">
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                <h3 className={`text-center text-xl font-bold tracking-widest text-cyan-100 uppercase md:text-left ${rubik.className}`}>
                  Foto-galleri
                </h3>
                <p className="mt-2 text-sm text-cyan-50/75 md:text-base">
                  Billeder indsendt under den afsluttede foto-mission.
                </p>
              </div>
              <div className="inline-flex items-center rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/85">
                {galleryAnswers.length} billeder
              </div>
            </div>

            <div className="mt-6 grid auto-rows-[180px] grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {galleryAnswers.map((answer, index) => {
                ...
                return (
                  <button
                    key={`finished-photo-${answer.id}`}
                    type="button"
                    onClick={() => setSelectedPhoto(answer)}
                    className={`group relative overflow-hidden rounded-4xl border border-white/10 bg-slate-950 text-left shadow-[0_24px_50px_rgba(2,6,23,0.4)] transition hover:-translate-y-1 hover:border-cyan-200/35 ${
                  ...
                  >
                    ...
                    />
                    <div className="pointer-events-none absolute inset-0 bg-linear-to-t from-slate-950 via-slate-950/10 to-transparent" />
                    <div className="absolute inset-x-0 bottom-0 p-4">
                      <div className="inline-flex max-w-full flex-wrap items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-50/85 backdrop-blur-md">
                        <span>{getPhotoLabel(answer)}</span>
                      </div>
      ...
      {selectedPhoto?.image_url ? (
        <div
          className="fixed inset-0 z-[1400] flex items-center justify-center bg-slate-950/90 p-4 backdrop-blur-md"
          onClick={() => setSelectedPhoto(null)}
        >
          <div
            className="w-full max-w-6xl overflow-hidden rounded-4xl border border-white/10 bg-slate-900/95 shadow-[0_40px_120px_rgba(2,6,23,0.7)]"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 border-b border-white/10 px-6 py-5">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-cyan-200/75">
                  Afsluttet foto-mission
                </p>
                <h4 className={`mt-2 text-2xl font-black uppercase tracking-[0.14em] text-white ${rubik.className}`}>
                  {selectedPhoto.studentName}
                </h4>
                <p className="mt-2 text-sm text-slate-300">{getPhotoLabel(selectedPhoto)}</p>
              </div>
              <button
                type="button"
                onClick={() => setSelectedPhoto(null)}
