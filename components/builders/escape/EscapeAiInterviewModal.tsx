...
const ESCAPE_AI_INTERVIEW_SESSION_KEY = "escape_ai_interview_state";
...
function normalizePuzzleCount(value: unknown): (typeof QUESTION_COUNT_OPTIONS)[number] {
...
export default function EscapeAiInterviewModal({
  ...
  useEffect(() => {
    ...
    const restoredDraft = readSessionDraft<SessionDraftState>(ESCAPE_AI_INTERVIEW_SESSION_KEY);
    ...
    setPuzzleCount(normalizePuzzleCount(restoredDraft?.puzzleCount));
    setError(null);
  ...
  useEffect(() => {
    ...
    writeSessionDraft(ESCAPE_AI_INTERVIEW_SESSION_KEY, {
      step: step === 5 ? 4 : step,
      topic,
      audience,
      tone,
  ...
  }, [audience, open, puzzleCount, step, tone, topic]);
