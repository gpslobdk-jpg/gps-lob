...
drop function if exists public.resolve_stratego_duel(
  uuid,
  uuid,
  uuid,
  double precision,
...
create or replace function public.stratego_haversine_meters(
...
revoke all on function public.stratego_haversine_meters(
  double precision,
  double precision,
  double precision,
  double precision
...
begin
  ...
  if v_distance_between_players > 20 then
    raise exception 'Målet er for langt væk (over 20m).';
  end if;
  ...
  if v_distance_defender_to_base <= 30 then
    raise exception 'Målet befinder sig i en fredszone.';
  end if;
