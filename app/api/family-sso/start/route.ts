...
function redirectToSkoleGpsLogin(
  requestUrl: URL,
  audience: FamilySsoAudience,
  requestId: string,
) {
  const continuation = new URL("/api/family-sso/start", requestUrl.origin);
  continuation.searchParams.set("request", requestId);
  if (audience !== "dagenstavle") continuation.searchParams.set("audience", audience);
  const target = new URL("/login", requestUrl.origin);
  target.searchParams.set("next", `${continuation.pathname}${continuation.search}`);
  const response = NextResponse.redirect(target, 303);
  response.headers.set("Cache-Control", "private, no-store, max-age=0");
  response.headers.set("Pragma", "no-cache");
  response.headers.set("Referrer-Policy", "no-referrer");
...
export async function GET(request: Request) {
  ...
  if (userError || !user?.id) {
    return audience === "printmitarbejdsark"
      ? redirectToSkoleGpsLogin(requestUrl, audience, requestId)
      : redirectToDestination(destinationOrigin, audience, requestId, "login");
  }
