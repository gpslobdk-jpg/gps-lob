...
const groups = {
  planlaeg: {
    ...
    icon: Calendar,
    tools: [
      { title: "DagensTavle", description: "Fra skema til tavle – klar til undervisning.", href: `${dagensTavleOrigin}/auth/family-sso/start?next=%2Ftavle&source=skolegps`, icon: Presentation, cta: "Åbn DagensTavle", tone: "yellow" },
      { title: "Årsplan", description: "Lav og tilpas en årsplan – klar som PDF.", href: "/dashboard/laerervaerktoejer/aarsplan-generator", icon: Calendar, cta: "Lav årsplan", tone: "sand" },
      { title: "SkemaPilot", description: "Byg en skemakladde med fag, lærere og lokaler.", href: "https://www.skemapilot.dk/app", icon: School, cta: "Åbn SkemaPilot", tone: "blue" },
      { title: "PrintMitArbejdsark", description: "Lav arbejdsark klar til print på få minutter.", href: `${printMitOrigin}/auth/family-sso/start?next=%2Flav&source=skolegps`, icon: FileText, cta: "Åbn arbejdsark", tone: "green" },
      { title: "KildeGPS", description: "Find udvalgte kilder til elevernes research.", href: "https://www.kildegps.dk", icon: Compass, cta: "Åbn KildeGPS", tone: "blue" },
    ],
  },
  aktiver: {
    ...
    icon: UsersRound,
    tools: [
      { title: "Skak", description: "Lær, vis og spil fysisk skak i klassen.", href: "/dashboard/laerervaerktoejer/skak", icon: ChessKnight, cta: "Åbn Skak", tone: "yellow" },
    ],
  },
  ud: {
    ...
    icon: Compass,
    tools: [
      { title: "Fysisk Stjerneløb", description: "Lav et analogt løb til print med poster i terrænet.", href: "/dashboard/opret/stjerneloeb", icon: Compass, cta: "Lav stjerneløb", tone: "green" },
    ],
  },
...
function ToolCard({ tool }: { tool: Tool }) {
  ...
  const className = "skolegps-adventure-card group flex min-h-48 flex-col rounded-2xl border border-sky-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-400 hover:shadow-[0_18px_38px_rgba(7,26,58,0.11)] focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-sky-500";
  const content = (
    <>
      <span className="skolegps-adventure-card-icon inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-sky-100 text-sky-800">
  ...
  return tool.href.startsWith("http") ? (
    <a href={tool.href} target="_blank" rel="noopener noreferrer" data-adventure-tone={tool.tone} className={className}>
      {content}
    </a>
  ) : (
    <Link href={tool.href} data-adventure-tone={tool.tone} className={className}>
      {content}
    </Link>
  );
}
            ...
            {(Object.entries(groups) as [GroupKey, (typeof groups)[GroupKey]][]).map(([key, group]) => {
              ...
              return (
                <Link
                  ...
                  data-adventure-tone={key === "planlaeg" ? "sand" : key === "aktiver" ? "yellow" : "green"}
                  className="skolegps-teacher-surface skolegps-adventure-card group rounded-3xl p-6 transition hover:-translate-y-0.5 hover:border-sky-400 hover:shadow-[0_20px_44px_rgba(7,26,58,0.12)] focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-sky-500"
                >
                  <span className="skolegps-adventure-card-icon inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-100 text-sky-800">
