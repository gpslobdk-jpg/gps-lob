...
function CategoryCard({ cat, active, onClick }: { cat: CategoryDef; active?: boolean; onClick: (key: string) => void }) {
  return (
    <div className="block w-full">
      <motion.article
        whileHover={{ y: -4, scale: 1.012 }}
        onClick={() => onClick(cat.key)}
        className={`${cardBaseClass} ${cat.accentClass} ${active ? "ring-4 ring-white/12" : ""} cursor-pointer`}
      >
...
export default function LibraryClient({ items: initialItems }: { items: Item[] }) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const newItem: Item = {
        id: data.item?.id ?? `tmp-${Date.now()}`,
        file_path: data.item?.file_path ?? data.item?.filePath ?? "",
        original_name: data.item?.original_name ?? data.item?.originalName ?? selectedFile.name,
        ai_title: data.item?.ai_title ?? data.item?.aiTitle ?? (data.item?.title ?? "Ny PDF"),
        category: data.item?.category ?? selectedCategory ?? "",
        created_at: data.item?.created_at ?? new Date().toISOString(),
        publicUrl: data.item?.publicUrl ?? data.publicUrl ?? null,
      };

      // if no publicUrl returned, build fallback from NEXT_PUBLIC_SUPABASE_URL
      if (!newItem.publicUrl && typeof process !== "undefined") {
        const supa = process.env.NEXT_PUBLIC_SUPABASE_URL;
        if (supa) {
          newItem.publicUrl = `${supa.replace(/\/$/, "")}/storage/v1/object/public/stjerneloeb_pdfs/${encodeURIComponent(newItem.file_path)}`;
        }
      }
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      ...
      <p className="mt-4 text-sm text-white/80">Vælg en kategori for at filtrere. Klik et kort for at se løb i den kategori.</p>

      <section className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        {CATEGORIES.map((cat) => (
          <CategoryCard key={cat.key} cat={cat} active={selectedCategory === cat.key} onClick={(k) => setSelectedCategory((prev) => (prev === k ? null : k))} />
        ))}
      </section>

      <section className="mt-10">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold">{selectedCategory ? `Viser: ${selectedCategory}` : `Alle Stjerneløb (${items.length})`}</h2>
          {selectedCategory ? (
            <button onClick={() => setSelectedCategory(null)} className="text-sm text-white/80 underline">
              Vis alle
            </button>
          ) : null}
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="mt-6 flex gap-6 overflow-x-auto py-6">
            {filtered.map((it) => (
              <article key={it.id} className={`${cardBaseClass} min-w-[18rem]`}>
                <div className={cardBackgroundShellClass}>
                  <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-b from-white/6 to-transparent" />
                  <div className="absolute inset-[1px] rounded-[1.95rem]" />
                </div>

                <div className={`${cardPanelClass} text-white`}>
                  <div className="flex h-full w-full flex-col items-center justify-center text-center">
                    <h3 className="text-lg font-black">{it.ai_title || it.original_name}</h3>
                    <p className="mt-2 text-xs text-white/80">{it.original_name}</p>

                    <div className="mt-4 flex gap-2">
                      {it.publicUrl ? (
                        <a href={it.publicUrl} target="_blank" rel="noreferrer" className="rounded-full border border-white/12 bg-white/8 px-3 py-2 text-sm font-medium text-white/90">
                          Åbn PDF
                        </a>
                      ) : null}
                      ...
                      <button
                        onClick={() => {
                          // Select — navigate to builder with query param
                          try {
                            window.location.href = `/dashboard/opret/stjerneloeb?library=${encodeURIComponent(it.id)}`;
                          } catch {
                            setSelectedId(it.id);
                          }
                        }}
                    ...
                    {selectedId === it.id ? <p className="mt-3 text-emerald-300">Valgt</p> : null}
                  </div>
                </div>
              </article>
            ))}
          ...
          <div role="dialog" aria-modal="true" className="relative w-full max-w-md rounded-[1rem] border p-6 bg-white/6">
            ...
            <div className="mt-4 space-y-3">
              ...
              <select
                ...
                disabled={isLoading}
              >
                <option value="">Vælg eller lad stå tom</option>
                {CATEGORIES.map((c) => (
