...
type PendingDanskAiReviewDraft = DanskAiInterviewDraft & {
...
const toolsTriggerButtonClass =
  "inline-flex items-center justify-center gap-2 rounded-[1.2rem] border border-rose-500/25 bg-rose-950/30 px-4 py-2.5 text-sm font-semibold text-rose-50 transition hover:bg-rose-900/35 disabled:cursor-not-allowed disabled:pointer-events-none disabled:opacity-50";

const toolsMenuItemClass =
  "flex w-full items-start gap-3 rounded-[1.25rem] px-4 py-3 text-left text-rose-50 transition hover:bg-rose-400/10 disabled:cursor-not-allowed disabled:opacity-50";
...
function normalizePendingDanskAiReviewDraft(value: unknown): PendingDanskAiReviewDraft | null {
  if (!isRecord(value)) return null;

  const title = asTrimmedString(value.title);
  const gradeLevels = normalizeGradeLevels(value.gradeLevels);
  const danishTopic = asTrimmedString(value.danishTopic);
  ...
  const questions = questionCandidates
    ...
    .filter((candidate): candidate is PendingDanskAiReviewDraft["questions"][number] => candidate !== null);
  ...
  return {
    subject: asTrimmedString(value.subject) || DANISH_SUBJECT,
    title,
    questions,
    gradeLevels,
    danishTopic,
...
function OpretDanskLoebPageContent() {
  ...
  const [pendingAiReviewDraft, setPendingAiReviewDraft] = useState<PendingDanskAiReviewDraft | null>(null);
  ...
  const applyDraftState = (draft: BuilderDraftState) => {
    ...
    const restoredPendingAiReviewDraft = normalizePendingDanskAiReviewDraft(draft.pendingAiReviewDraft);
  ...
  const scrollToQuestionCard = (id: number) => {
    ...
    document.getElementById(`dansk-post-${id}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
  };
  ...
  const handleAiInterviewComplete = (draft: DanskAiInterviewDraft) => {
    ...
    setPendingAiReviewDraft({
      ...
      danishTopic: draft.danishTopic.trim(),
  ...
  const applyAiReviewDraft = () => {
    ...
    setNotice({
      tone: "success",
      message: "Et komplet udkast er klar til dit dansk-løb. Gennemgå felterne og placer posterne på kortet.",
    });
  };
  ...
  return (
    <>
      <div className={`relative min-h-screen bg-slate-950 text-rose-50 print:h-auto print:min-h-0 print:overflow-visible print:bg-white print:text-black ${poppins.className}`}>
        <div className="fixed inset-0 -z-10 bg-linear-to-br from-rose-950/60 via-slate-900/80 to-rose-900 backdrop-blur-[2px] print:hidden" />
                    ...
                    <div className="relative z-40 rounded-4xl border border-rose-500/30 bg-rose-950/20 p-5 shadow-[0_18px_40px_rgba(0,0,0,0.24)] backdrop-blur-2xl sm:p-6">
                      <div className="flex flex-col gap-4">
                        ...
                        <input
                          ...
                          disabled={isEditorBusy}
                          placeholder="F.eks. 5.A's store laeseloeb"
...
"
                ...
                <div className="relative z-0 rounded-4xl border border-rose-500/30 bg-rose-950/20 p-5 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-2xl sm:p-6">
                  ...
                  <div ref={saveFeedbackRef} className="mt-6 space-y-4">
                    ...
                    <button
                      ...
                      disabled={isSaving}
                      className={`w-full rounded-[1.6rem] border border-rose-500/30 bg-rose-600 px-6 py-4 text-lg font-extrabold uppercase tracking-[0.22em] text-slate-950 shadow-lg shadow-rose-500/20 transition-all duration-300 hover:bg-rose-500 disabled:cursor-not-allowed disabled:pointer-events-none disabled:opacity-50 ${
                        shouldHighlightSave
                          ? "scale-105 ring-4 ring-rose-500 ring-offset-2 ring-offset-slate-950 shadow-rose-500/50"
                          : ""
                      }`}
                    >
          ...
          <aside className="hidden w-full p-4 pt-0 sm:px-6 lg:block lg:w-[48%] lg:p-8 lg:pl-0 print:hidden">
            <div className="lg:sticky lg:top-20">
              <div className="h-[42vh] min-h-80 w-full overflow-hidden rounded-4xl border border-rose-500/20 bg-slate-900/50 shadow-[0_0_0_1px_rgba(244,63,94,0.08),0_0_36px_rgba(244,63,94,0.08),0_24px_60px_rgba(0,0,0,0.38)] backdrop-blur-2xl lg:h-[calc(100vh-(--spacing(28)))]">
                <MapPicker
                  center={mapCenter}
                  pins={pins}
                  zones={previewZones}
          ...
          <PortalMenu
            ...
            className="w-[min(22rem,calc(100vw-3rem))] max-h-[min(24rem,calc(100vh-2rem))] overflow-x-hidden overflow-y-auto rounded-[1.6rem] border border-rose-400/20 bg-slate-950/96 p-2 shadow-[0_28px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl overscroll-contain"
          >
            <button
              type="button"
              onClick={() => {
              ...
              disabled={isEditorBusy}
              className="flex w-full items-start gap-3 rounded-[1.25rem] px-4 py-3 text-left text-rose-50 transition hover:bg-rose-400/10 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-rose-400/20 bg-rose-400/10 text-rose-200">
              ...
              <span>
                ...
                <span className="mt-1 block text-sm leading-6 text-rose-100/72">
                  Tilføj en tom dansk-post og byg den videre fra bunden.
                </span>
              </span>
            </button>
      ...
      {pendingAiReviewDraft ? (
        <AiReviewDraftModal
          ...
          summaryItems={[
            ...
            { label: "Fag", value: pendingAiReviewDraft.subject || DANISH_SUBJECT },
            { label: "Klassetrin", value: pendingAiReviewGradeLabel },
            { label: "Antal poster", value: pendingAiReviewDraft.questions.length },
          ]}
          detailItems={[
            { label: "Danskfagligt emne", value: pendingAiReviewDraft.danishTopic || "Ikke angivet" },
          ]}
